#!/usr/bin/env python
"""nbreport microservice framework.
"""
...
def server(run_standalone=False):
    ...
    """
    # Add "/nbreport" for mapping behind api.lsst.codes
    app = APIFlask(name="uservice-nbreport",
                   version="0.0.1",
    ...
    @app.route("/nbreport")
    @app.route("/nbreport/")
    # @app.route("/nbreport/<parameter>")
    # or, if you have a parameter, def route_function(parameter=None):
    def route_function():
        """Publication service for LSST notebook-based reports
        """
